...
def calc_magical_damage(base_dmg: int, atk: int, defense: int) -> int:
    ...
    return max(1, base_dmg + int(atk * 0.5) - int(defense * 0.5) + random.randint(-1, 3))
...
def calc_element_multiplier(atk_element: str, def_element: str) -> float:
    ...
    element_map = {
        ('金', '木'): 1.5, ('木', '土'): 1.5, ('土', '水'): 1.5,
        ('水', '火'): 1.5, ('火', '金'): 1.5,
        ('木', '金'): 0.7, ('土', '木'): 0.7, ('水', '土'): 0.7,
        ('火', '水'): 0.7, ('金', '火'): 0.7,
    }
...
def calc_status_application(status_name: str, status_level: int = 1) -> dict:
    ...
    statuses = {
        ...
        '燃烧': {'per_turn_dmg': status_level * 3, 'def_reduce': 0.20, 'duration': 5, 'max_stack': 3},
...
def roll_spirit_root() -> dict:
    ...
    ELEMENT_COUNTER = {'金': ('克木', '被火克'), '木': ('克土', '被金克'), '土': ('克水', '被木克'), '水': ('克火', '被土克'), '火': ('克金', '被水克')}
    ...
    return {'name': display_name, '灵根类型': name, '灵根属性': elements,
            '五行亲和': affinity, '五行相克': counter, 'stats': stats}
...
def roll_physique(spirit_root_name: str, birthplace: str = '') -> dict:
    ...
    birthplace_bonuses = {
        '绝境险地': 0.15, '古战场遗址旁': 0.10, '灵气浓郁之地': 0.08,
        '妖兽山脉边缘': 0.05, '边境战区': 0.05,
    }
...
def generate_physique_detail() -> dict:
    ...
    # 维度1：来源类型 (2.2.2)
    sources = [
        ('先天遗传', 0.30, {'name_prefix': ('先天', '玄', '天'), 'suffix': ('之体', '之体', '之骨')}),
        ('后天异变', 0.25, {'name_prefix': ('', '', ''), 'suffix': ('身', '躯', '体')}),
        ('血脉觉醒', 0.20, {'name_prefix': ('', '古', '灵'), 'suffix': ('血脉', '血脉', '后裔')}),
        ('灵魂特质', 0.15, {'name_prefix': ('不灭', '七窍', ''), 'suffix': ('魂', '心', '灵')}),
        ('天地馈赠', 0.08, {'name_prefix': ('天道', '道', '天'), 'suffix': ('之体', '之体', '胎')}),
        ('诅咒/异化', 0.02, {'name_prefix': ('诅咒', '厄', '灾'), 'suffix': ('之体', '之躯', '之身')}),
    ]
    source = weighted_choice(sources)

    directions = [
        ('修炼向', 0.25, {'speed_bonus': random.uniform(0.10, 0.50),
                          'breakthrough_bonus': random.uniform(-0.20, 0.20),
                          'qi_recovery': random.uniform(0.10, 0.30)}),
        ('战斗向', 0.25, {'atk_bonus': random.uniform(0.10, 0.40),
                          'crit_bonus': random.uniform(0.05, 0.15),
                          'element_dmg': random.uniform(0.20, 0.60)}),
        ('防御向', 0.15, {'def_bonus': random.uniform(0.10, 0.30),
                          'hp_bonus': random.uniform(0.15, 0.50),
                          'dmg_reduce': random.uniform(0.05, 0.20)}),
        ('恢复向', 0.12, {'heal_rate': random.uniform(0.20, 1.00),
                          'status_recovery': random.uniform(0.30, 0.80)}),
        ('感知向', 0.10, {'sense_bonus': random.uniform(0.20, 0.60),
...
def roll_origin() -> dict:
    ...
    families = [
        ('孤儿/弃婴', 15, {'财富': 0, '知识': 0, '人脉': 0, '描述': '你是孤儿出身，无父无母，来历成谜。'}),
        ('贫苦农家', 20, {'财富': 0, '知识': 0, '人脉': 1, '体魄加成': 5, '描述': '你出生在贫苦农家，从小劳作，练就一副结实身板。'}),
        ('猎户之家', 12, {'财富': 1, '知识': 1, '人脉': 1, '体魄加成': 10, '气血加成': 10, '物品': '短刀', '描述': '猎户家长大的你，跟父亲学过狩猎和草药辨识。'}),
        ('渔/樵民家', 10, {'财富': 0, '知识': 1, '人脉': 1, '地形适应': '水域/山地+10%', '描述': '渔樵为生的家庭让你熟悉山水之道。'}),
        ('商贾之家', 10, {'财富': 50, '知识': 2, '人脉': 1, '交易加成': 0.05, '描述': '商贾之家让你耳濡目染，懂得些买卖之道。'}),
        ('书香门第', 8, {'财富': 20, '知识': 3, '人脉': 1, '悟性加成': 5, '描述': '书香世家熏陶出你的聪慧，识字读书不在话下。'}),
        ('没落贵族', 6, {'财富': 30, '知识': 3, '人脉': 1, '物品': '祖传物品', '描述': '曾经的贵族已没落，但家中尚有几分底蕴。'}),
        ('武林世家', 5, {'财富': 20, '知识': 1, '人脉': 1, 'atk加成': 5, '体魄加成': 10, '物品': '凡器武器', '描述': '武林世家出身的你有一身好武艺。'}),
        ('江湖郎中/药师', 4, {'财富': 10, '知识': 3, '人脉': 1, '炼丹初始': 1, '描述': '跟着师父行医问药，你认得不少草药。'}),
        ('修仙小族(旁支)', 5, {'财富': 50, '知识': 3, '人脉': 2, '初始修为': '炼气一层', '物品': '基础功法', '描述': '修仙小族的旁支子弟，比凡人多了几分见识。'}),
        ('修仙大族(嫡系)', 2, {'财富': 200, '知识': 4, '人脉': 4, '初始修为': '炼气二层', '物品': '黄阶功法', '描述': '修仙大族嫡系子弟，资源丰厚但家族内斗激烈。'}),
        ('修仙大族(庶出)', 2, {'财富': 30, '知识': 2, '人脉': 0, '初始修为': '炼气一层', '描述': '庶出身份让你在修仙大族中步步艰辛。'}),
        ('被捡养/收养', 1, {'财富': 0, '知识': 0, '人脉': 1, '描述': '你被好心人捡回收养，养父母身份莫测。'}),
    ]
    ...
    birthplaces = [
        ...
        ('宗门属地村落', 0.15, {'环境特征': '受宗门庇护', '效果': {'每5年宗门收徒': True}}),
        ('边境战区', 0.05, {'环境特征': '常年战乱', '效果': {'死亡风险': '高', '遗落物品': True}}),
        ('妖兽山脉边缘', 0.05, {'环境特征': '危险与机遇并存', '效果': {'灵草概率': True, '妖兽威胁': True}}),
        ('古战场遗址旁', 0.03, {'环境特征': '怨气弥漫', '效果': {'残缺法器': True, '阴气侵蚀': True}}),
        ('灵气枯竭之地', 0.05, {'环境特征': '修炼困难', '效果': {'修炼速度': -0.20, '心境': 10}}),
        ('灵气浓郁之地', 0.04, {'环境特征': '得天独厚', '效果': {'修炼速度': 0.20, '觊觎风险': True}}),
        ('绝境险地', 0.03, {'环境特征': '极端环境', '效果': {'体魄': 15, '随机抗性': 0.20}}),
    ...
    if random.random() < 0.30:
        specials = [
            ('出生时天降异象', {'气运加成': 20, '描述': '你出生时天降异象，气运加身，但也被某势力暗中关注。'}),
            ('襁褓中放于宗门门口', {'物品': '信物/玉佩', '描述': '你被人放在修仙宗门门口，身边一块信物指向未知身世。'}),
            ('幼年大病不死', {'毒抗': 0.10, '寿元修正': -5, '描述': '幼年一场大病差点要了你的命，但挺过来了，毒抗有所提升。'}),
            ('被路过高人点拨', {'悟性加成': 10, '心境加成': 5, '描述': '一位路过的修士随口点拨了你一句，令你茅塞顿开。'}),
            ('捡到残破玉简', {'物品': '残缺功法/法术', '描述': '你偶然捡到一枚残破玉简，内含一门残缺的修炼之法。'}),
            ('家族被灭门', {'财富': 100, '心境修正': -20, '业力初始': 5, '描述': '你的家族被仇家灭门，你带着遗产独自逃生，心中埋下仇恨。'}),
            ('被当成炉鼎培养', {'灵气加成': 0.30, '寿元修正': -20, '描述': '你从小被当成炉鼎培养，灵气基础扎实但折损了寿元。'}),
            ('出生在古墓/遗迹中', {'物品': '古物', '诅咒': True, '描述': '你在古墓中出生，身边放着一件神秘古物，但似乎也招来了某种诅咒。'}),
            ('天生阴阳眼', {'神识加成': 15, '描述': '你天生阴阳眼，能看见常人看不到的灵体鬼魂，但也容易招惹脏东西。'}),
            ('被妖兽养大', {'体魄加成': 20, '妖兽亲和': 0.50, '描述': '你从小被妖兽养大，野性未脱，体魄强健但不懂人类礼法。'}),
        ]
...
def calc_alchemy(level: int, wu_xing: int, herb_q: str = '普通',
                 years: int = 0, furnace: str = '法器', fire: str = '地火',
                 aux: int = 0, phys_bonus: float = 0.0) -> str:
...
def calc_alchemy_success_rate(level: int, wu_xing: int, herb_q: str = '普通',
                                years: int = 0, furnace: str = '法器',
                                fire: str = '地火', aux: int = 0,
                                phys_bonus: float = 0.0) -> dict:
...
def calc_forging(level: int, wu_xing: int, material_q: str = '普通',
                 fire: str = '地火', phys_bonus: float = 0.0) -> str:
...
def calc_mind_effect(mind: int) -> dict:
    ...
    if mind >= 80:
        return {'label': '平和', 'breakthrough_bonus': 0.10, 'immune_heart_demon': False}
    if mind >= 60:
        return {'label': '正常', 'breakthrough_bonus': 0, 'immune_heart_demon': False}
    if mind >= 40:
        return {'label': '烦躁', 'breakthrough_bonus': -0.10, 'demon_chance_bonus': 0.05}
    if mind >= 20:
        return {'label': '混乱', 'breakthrough_bonus': -0.20, 'demon_chance_bonus': 0.15}
    return {'label': '疯魔', 'breakthrough_bonus': 0, 'can_cultivate': False, 'self_harm_chance': 0.50}
...
def calc_possess(attacker_sense: int, target_sense: int,
                 target_has_protection: bool = False) -> tuple:
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        ...
        print("可用函数:")
        funcs = [name for name in dir() if name.startswith(('calc_', 'roll_', 'generate_'))]
